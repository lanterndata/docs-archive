...
The size of the dynamic list for the nearest neighbors (used during search). Higher `ef` leads to more accurate but slower search. (this parameter is also controlled via seesion based `lantern_hnsw.ef_search` variable)

  Maximum allowed value for `ef` is 400
  
  <Note>
  This parameter is also controlled via session based `lantern_hnsw.ef_search` variable, which has precedence over this parameter
  `sql SET lantern_hnsw.ef_search=128`
  </Note>
</ResponseField>
...
<ResponseField name="lantern_hnsw.init_k" type="string" default="10">
  ...
  ```
  You should set the `lantern_hnsw.init_k` to 100.
...
<ResponseField name="lantern_hnsw.ef_search" type="string" default="64">
  ...
  Maximum allowed value for `lantern_hnsw.ef_search` is 400
  </Note>
</ResponseField>
