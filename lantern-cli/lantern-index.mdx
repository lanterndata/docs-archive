...
With Lantern CLI you can create HNSW index for your vector data locally without using any third party APIs.

## Prerequisites
- [Lantern CLI](/lantern-cli/overview)
- Running Postgres database with [Lantern extension](/get-started/overview) installed
...
## Run Index Creation
```bash
lantern-cli create-embeddings  --model 'microsoft/all-MiniLM-L13-v2'  --uri 'postgresql://[username]:[password]@localhost:5432/[db]' --table "embeddings" --column "v" -m 10 --efc 128 --ef 64 --metric-kind l2sq --out /tmp/index.usearch
```

<Note>
If you have generated index file in a different server, copy the index file to your database server in a location where the user running database will have the necessarry permissions to access the file.
You can put it in `/tmp/index.usearch` and run `chmod 766 /tmp/index.usearch` on it.
</Note>
...
Usage: lantern-cli create-index [OPTIONS] --uri <URI> --table <TABLE> --column <COLUMN> -d <DIMS>
