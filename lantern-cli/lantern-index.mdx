...
<Note>
  If you have generated index file in a different server, copy the index file to
  your database server in a location where the user running database will have
  the necessarry permissions to access the file. You can put it in
  `/tmp/index.usearch` and run `chmod 766 /tmp/index.usearch` on it.
</Note>
...
<Note>
  You can read more about the benchmarking and performance of Lantern Index in
  [this](https://lantern.dev/blog/hnsw-index-creation) blog post.
</Note>
